...
import {
  ...
  VESSEL_CERTIFICATE_DASHBOARD_ROWS,
...
export default function VesselCertificateDashboardWindow({ onMinimize, onClose, preview = false }) {
  ...
  const [rows, setRows] = useState(VESSEL_CERTIFICATE_DASHBOARD_ROWS)
  ...
  const handleReset = () => {
    ...
    setSelectedBookmark('')
    setRows(VESSEL_CERTIFICATE_DASHBOARD_ROWS)
    setSelectedRow(0)
...
function applyFilters(filters) {
  return VESSEL_CERTIFICATE_DASHBOARD_ROWS.filter((row) => {
