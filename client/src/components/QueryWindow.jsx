        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-white">
          ...
          <div className="min-h-0 flex-1 overflow-hidden">
            <div className="flex h-full min-h-0 flex-col bg-white">
              ...
              >
                <GridTable columns={columns} rows={rows} className={secondaryColumns ? 'h-full' : 'flex-1'} />
              </div>
...
function GridTable({ columns, rows, className = '' }) {
  ...
  return (
    <div className={`overflow-auto bg-white ${className}`}>
      <div style={{ minWidth }}>
        ...
        <GridRows columns={columns} rows={rows} />
      </div>
    </div>
  )
}
...
function GridRows({ columns, rows }) {
  if (!rows.length) {
