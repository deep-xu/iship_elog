...
import { getCounterAllowed, setCounterAllowed } from '@/stores/counterAllowedStore.js'
...
function TabBody({ tab, detail }) {
  ...
  if (tab.key === 'counter') {
    return <CounterTab detail={detail} />
  }
  ...
  if (tab.key === 'standardJobs') {
    ...
    // same equipment.
    const rows = findRelatedJobsForEquipment(detail.name, detail.category).map((row) => [
      row.jobSize ?? '',
      row.jobName ?? '',
      row.interval ?? '',
      row.lastDone ?? '',
...
function CounterTab({ detail }) {
  // Every equipment's counter starts as "Not Allow"; it is only on once the
  // user explicitly picks "Allow Counter" here.
  const [allowed, setAllowed] = useState(() => getCounterAllowed(detail.name) ?? false)

  function choose(next) {
    setAllowed(next)
    setCounterAllowed(detail.name, next)
  }

  return (
    <div className="rounded-[8px] border border-[#e0eaf1] bg-white p-[16px]">
      <div className="flex flex-wrap items-center gap-x-[10px] gap-y-[6px] pb-[12px]">
        <span className="text-[12px] font-semibold text-[#5b7690]">Counter:</span>
        <button
          type="button"
          onClick={() => choose(true)}
          className="flex items-center gap-[6px] text-[12px] text-ns-navy focus:outline-none"
        >
          <span className={`flex h-[14px] w-[14px] items-center justify-center rounded-full border ${allowed ? 'border-ns-blue' : 'border-[#c7d6e0]'}`}>
            {allowed ? <span className="h-[7px] w-[7px] rounded-full bg-ns-blue" /> : null}
          </span>
          Allow Counter
        </button>
        <button
      ...
      {!allowed && (
        <p className="text-[12px] text-[#8aa0b4]">
          This equipment's counter is switched off — it will be hidden from Running Hours.
        </p>
      )}
    </div>
  )
