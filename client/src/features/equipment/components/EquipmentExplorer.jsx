...
import AddEquipmentDialog from '@/features/equipment/components/AddEquipmentDialog.jsx'
import EquipmentDetailWindow from '@/features/equipment/components/EquipmentDetailWindow.jsx'
import PartDetailWindow from '@/features/equipment/components/PartDetailWindow.jsx'
import { FIND_EQUIPMENT_SECTIONS } from '@/data/findEquipment.js'
import { FIND_PART_SECTIONS } from '@/data/findPart.js'
import { saveDocument } from '@/stores/documentsStore.js'
...
export default function EquipmentExplorer({ initialMode = 'browse', onMinimize, onClose, preview = false }) {
  const [tree, setTree] = useState(getEquipmentTree)
  ...
  function handleAddNode(parentId, draft) {
    ...
    setTree(result.tree)
    cacheEquipmentTree(result.tree)
  ...
  function handleSaveForm(formMode) {
    ...
    const label = formMode === 'equipment' ? 'Find Equipment' : 'Find Part'
    setStatusMessage(
      saveDocument(storageKey, payload) ? `${label} search saved.` : 'Saving is unavailable.',
    )
  }
